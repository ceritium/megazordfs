...
const { virtualDevice } = require('./volumeDevice')
...
const handlers = function (volume) {
  ...
  const vDevice = virtualDevice(volume)
  const replication = Replication(volume, vDevice)
  ...
  return {
    ...
    write: function (path, fd, buffer, length, position, cb) {
      ...
      Promise.all(fds.map((fdBlock) => {
        new Promise((resolve) => {
          fs.write(fdBlock, cloned, 0, length, position, (err) => {
    ...
    statfs: (path, cb) => {
      ...
      // TODO: only count files, no folders
      dbEntries.count({ volume: volumeName }, (_err, count) => {
        cb(ZERO, {
          bsize: BSIZE,
          // frsize: 20,
          blocks: vDevice.totalSize() / BSIZE,
          bfree: vDevice.freeSpace() / BSIZE,
          bavail: vDevice.available() / BSIZE,
          files: count
          // ffree: check.free / BSIZE,
          // favail: check.available / BSIZE,
          // fsid: 42,
          // flag: 1000000,
          // namemax: 1000000
        })
