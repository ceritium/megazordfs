...
const handlers = function (volume) {
  ...
  return {
    ...
    write: function (filePath, fd, buffer, length, offset, cb) {
      console.log('write', filePath, fd, length, offset)

      const pathBlock1 = path.join(volume.blocks[0], filePath)
      const pathBlock2 = path.join(volume.blocks[1], filePath)

      console.log(pathBlock1)
      const fd1 = fs.openSync(pathBlock1, 'w')
      fs.writeSync(fd1, buffer, offset, 0)
      // const fd2 = fs.openSync(pathBlock2, 'w')
      // fs.writeSync(fd2, buffer, offset, length)
    ...
    read: function (filePath, fd, buffer, length, offset, cb) {
      console.log('read', filePath, fd, length, offset)

      const pathBlock1 = path.join(volume.blocks[0], filePath)
      const fdMain = fs.openSync(pathBlock1, 'r')

      fs.readSync(fdMain, buffer, offset, length)
      return cb(length)
    },
