    ...
    unlink: (filePath, cbResult) => {
      dbEntries.findOne({ path: filePath, volume: volumeName }, (_err, entry) => {
        dbEntries.remove(
          ...
          { multi: true },
          (_err) => {
            cbBlocks(entry.blocks, filePath, (fileBlockPath) => {
    ...
    write: (filePath, cbWrite, cbResult) => {
      dbEntries.findOne({ path: filePath, volume: volumeName }, (_err, entry) => {
        if (entry && entry.blocks) {
          // If the entry has blocks assigned, run it per block
          cbBlocks(entry.blocks, filePath, cbWrite)
          ...
          // Otherwise, define some blocks, run the cb and upsert the entry with the blocks
